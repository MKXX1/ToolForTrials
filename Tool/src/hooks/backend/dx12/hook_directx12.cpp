        ...
        if (SUCCEEDED(pSwapChain->GetDevice(IID_PPV_ARGS(&g_pd3dDevice)))) {
            {
                ...
                // ImGui::GetIO().Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\seguiemj.ttf", 16.0f, &cfg, ImGui::GetIO().Fonts->GetGlyphRangesCyrillic());
                ImGui::GetIO().Fonts->AddFontFromMemoryCompressedBase85TTF(playe, 17.0f, &cfg, ImGui::GetIO().Fonts->GetGlyphRangesCyrillic());
